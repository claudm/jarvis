import nltk
from . import nodebox_linguistics_extended
import string
# import sets
import unicodedata
import random
...
def verbToNoun(verb):
	nounsTemp = []
	for val in [(w, w.derivationally_related_forms()) for w in nltk.corpus.wordnet.lemmas(nodebox_linguistics_extended.verb.infinitive(verb))]:
...
def autoWordToNoun(rawword):
	word = rawword
	infVerb = nodebox_linguistics_extended.verb.infinitive(rawword)
	if infVerb!='' and nodebox_linguistics_extended.is_verb(infVerb):
		word = infVerb 
	nounsTemp = []
	for val in [(w, w.derivationally_related_forms()) for w in nltk.corpus.wordnet.lemmas(word)]:
		nounsTemp.append(set([(w.name) for w in val[1] if '.n.' in str(w)]))
...
def autoWordToVerb(rawword):
	...
	nounsTemp = []
	for val in [(w, w.derivationally_related_forms()) for w in nltk.corpus.wordnet.lemmas(word)]:
		nounsTemp.append(set([(w.name) for w in val[1] if '.v.' in str(w)]))
...
def autoPlural(word):
	if nodebox_linguistics_extended.is_adjective(word):
		return nodebox_linguistics_extended.plural.adjective_plural(word)
	else:
		return nodebox_linguistics_extended.plural.noun_plural(word)

def autoLemmatize(word):
	wnl = nltk.stem.wordnet.WordNetLemmatizer()
	infVerb = nodebox_linguistics_extended.verb.infinitive(word)
	if infVerb!='' and nodebox_linguistics_extended.is_verb(infVerb):
...
def correctSpelling(phrase):
	...
	for word in phrase.split():
		suggestion = nodebox_linguistics_extended.spelling.suggest(word)
...
#print(phrase, correctSpelling(phrase))
#
#print(nodebox_linguistics_extended.sentence.tag(correctSpelling(phrase)))
