import nltk
import nlpy
import nodebox_linguistics_extended

# #import psw
# import gapi

# openAppList = {'firefox': 'firefox','calibre': 'calibre','notepad': 'notepad','editor': 'gedit', 'chrome':'chrome','console':'gnome-terminal', 'calculator':'gnome-calculator'}
# closeAppList = {'firefox': 'firefox','calibre': 'calibre','notepad': 'notepad','editor': 'gedit', 'chrome':'chrome', 'console':'gnome-terminal', 'calculator':'gnome-calculator'}
# # import player
# #
...
# 			sortedvals = sorted(iter(vals.items()), key=operator.itemgetter(1),reverse=True)
...
# 	global openAppList #= {'editor': 'gedit', 'chrome':'google-chrome', 'console':'gnome-terminal', 'calculator':'gnome-calculator'}
# 	prog = score[1]
# 	#psw.play(speech.getAudio(translator.translate('I will open the '+prog)))
# 	from os import system
# 	#system('nohup '+openAppList[prog]+' &')
# 	system("start " + openAppList[prog] )


# def doCloseProgram(text, score, speech, translator):
# 	global closeAppList #= {'editor': 'gedit', 'browser':'chrome', 'console':'gnome-terminal', 'calculator':'gnome-calculator'}
# 	prog = score[1]
# #	psw.play(speech.getAudio(translator.translate('I will close the '+prog)))
# 	from os import system
# 	#system('killall ' + ' ' + closeAppList[prog])
# 	system('taskkill /IM ' + closeAppList[prog]+".exe")
...
# 	global openAppList #= {'editor': 'gedit', 'chrome':'google-chrome', 'console':'gnome-terminal', 'calculator':'gnome-calculator'}
# 	global closeAppList #= {'editor': 'gedit', 'chrome':'chrome', 'console':'gnome-terminal', 'calculator':'gnome-calculator'}
...
# 		cmds = ['changeLanguage','openProgram','closeProgram']# ,'whatTime','whatDay','playSong','pauseSong','nextSong','previousSong','whatIs']
# 		#cmds = ['openProgram']# ,'whatTime','whatDay','playSong','pauseSong','nextSong','previousSong','whatIs']

# 		cmd = {}
# 		if ('change' in [item.lower() for item in tokens]):
		
# 			cmd['changeLanguage'] = changeLanguage(tokens, list(gapi.languages.keys()))
# 		if ('open' in [item.lower() for item in tokens]):
		
# 			cmd['openProgram'] = openProgram(tokens, list(openAppList.keys()))
# 		if ('close' in [item.lower() for item in tokens]):
	
# 			cmd['closeProgram'] = closeProgram(tokens, list(closeAppList.keys()))
